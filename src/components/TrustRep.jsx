...
function TrustRep() {
    ...
    const fetchTrustReports = async () => {
        ...
        try {
            ...
            const res = await axios.get(`https://backendinstursed.onrender.com/api/v1/TrustAccidentReport/all`, { // تأكد من هذا الـ Endpoint
                headers: { token }
            });
    ...
    const handleDelete = async (id) => {
        ...
        const token = `islam__${localStorage.getItem("token")}`;
        try {
            await axios.delete(`https://backendinstursed.onrender.com/api/v1/TrustAccidentReport/delete/${id}`, { // تأكد من هذا الـ Endpoint
                headers: { token }
            });
            fetchTrustReports();
            handleMenuClose();
